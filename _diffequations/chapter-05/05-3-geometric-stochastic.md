...
$$
\sigma_F = \int_0^T \vert D_t F \vert_H^2 dt = \int_0^T \sin^2\theta_T dt = T \sin^2\theta_T
$$
...
$$
\| F \|_{1,p} = \left( \mathbb{E}[\vert F \vert^p] + \mathbb{E}[\| DF \|_H^p] \right)^{1/p}
$$
...
$$
F = \mathbb{E}[F] + \int_0^T \mathbb{E}[D_t F \vert \mathcal{F}_t] dB_t
$$
...
$$
\mathbb{E}[D_t F \vert \mathcal{F}_t] = 2 \mathbb{E}[B_T \vert \mathcal{F}_t] = 2 B_t
$$
...
The hedging strategy is $\pi_t = \mathbb{E}[D_t F \vert \mathcal{F}_t]$, providing the exact portfolio allocation needed to replicate the Asian option payoff. This demonstrates how Malliavin calculus directly computes hedging strategies for path-dependent derivatives.
...
$$
p_t(x,y,z) \sim (4\pi t)^{-3/2} \exp\left(-\frac{x^2 + y^2}{4t} - \frac{\vert z - \frac{xy}{2} \vert^2}{t^3}\right)
$$
...
- [Previous Section: 5.2 SDEs & Diffusion Processes]({{ '/diffequations/chapter-05/05-2-sdes/' \vert relative_url }})
- [Next Section: 5.4 Rough Paths & Controlled Rough Paths]({{ '/diffequations/chapter-05/05-4-rough-paths/' \vert relative_url }})
