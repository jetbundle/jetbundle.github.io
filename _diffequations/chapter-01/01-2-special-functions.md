...
$$
 z(1-z)rac{d^2y}{dz^2} + ig[c - (a+b+1)zig]rac{dy}{dz} - ab\,y = 0.
$$

Inside $\lvert z
vert < 1$, the solution is the Gauss series

$$
{}_2F_1(a,b;c;z) = \sum_{n=0}^{\infty} rac{(a)_n (b)_n}{(c)_n}rac{z^n}{n!},
$$
...
The divergence records the **Stokes phenomenon**: as $rg z$ crosses certain rays, the coefficients of exponentially suppressed terms jump discontinuously. Controlling these jumps via connection formulae or Borel summation provides the analytic continuation machinery used later in the text (**Olver et al., 2010**).
...
$$
P_n(x) = {}_2F_1\left(-n,n+1;1;rac{1-x}{2}ight) = \sum_{k=0}^{n} rac{(-n)_k (n+1)_k}{k!}\left(rac{1-x}{2}ight)^k.
$$

For $n=2$,

$$
P_2(x) = 1 - 6\left(rac{1-x}{2}ight) + 3\left(rac{1-x}{2}ight)^2 = rac{3x^2 - 1}{2},
$$
...
$$
-u''(r) + \left[rac{\ell(\ell+1)}{r^2} - rac{2}{r}ight]u(r) = 2E\,u(r)
$$
...
Consider ${}_2F_1(a,b;c;2z/b)$ with $b	o\infty$. Since $(b)_n = b^n \left[1 + O\!\left(rac{1}{b}ight)ight]$, the series approaches

$$
{}_0F_1(c; z) = \sum_{n=0}^{\infty} rac{1}{(c)_n}rac{z^n}{n!}.
$$
...
$$
J_
u(x) = \sum_{k=0}^{\infty} rac{(-1)^k}{k!\,\Gamma(k+
u+1)}\left(rac{x}{2}ight)^{2k+
u},
$$
...
$$
f(r) = \sum_{n=1}^{\infty} c_n J_0\left(rac{\lambda_n r}{a}ight), \qquad \lambda_n = 	ext{nth zero of }J_0,
$$

and using the weight $w(r)=r$ gives

$$
\int_{0}^{a} r\, J_0\left(rac{\lambda_m r}{a}ight) J_0\left(rac{\lambda_n r}{a}ight) dr = rac{a^2}{2} J_1^2(\lambda_n)\,\delta_{mn}.
$$

Hence

$$
 c_n = rac{2}{a^2 J_1^2(\lambda_n)} \int_{0}^{a} r\, f(r)\, J_0\left(rac{\lambda_n r}{a}ight) dr,
$$
