...
- Contour deformation is essential for capturing all relevant contributions.
- The leading contribution comes from the global minima of the phase function.
...
Matching through the turning point introduces the $\pi/4$ phase shift, connecting the oscillatory solution on one side to the appropriate linear combination on the other.

**Key Insights:**
- Turning points require local analysis beyond WKB.
- Airy functions provide the universal transition functions near simple turning points.
- The $\pi/4$ phase is a topological invariant of the turning point structure.
...
This gives scaling $I(\lambda) \sim \lambda^{-1/4}$ instead of $\lambda^{-1/2}$, with the constant determined by the quartic coefficient.

**Key Insights:**
- Degenerate stationary points produce different power-law scalings.
- The asymptotic order depends on the first non-vanishing derivative.
- Multiple degenerate points may contribute at the same order, requiring careful analysis.
...
This exponentially small difference is the Stokes jump. The Stokes multiplier is $2\pi i e^{-1}$, encoding how the subdominant exponential is activated when crossing the Stokes line.

**Key Insights:**
- Borel summation provides a rigorous interpretation of divergent series.
- Stokes jumps are encoded in the analytic continuation of the Borel transform.
- The multiplier is determined by the singularity structure in the Borel plane.
