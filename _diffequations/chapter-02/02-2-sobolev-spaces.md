...
$$
\int_{\Omega} u(x) D^{\alpha}\phi(x)\,dx = (-1)^{\mid \alpha \mid} \int_{\Omega} v(x)\phi(x)\,dx
$$

for all $\phi \in C_{c}^{\infty}(\Omega)$. If the classical derivative exists, it coincides with the weak derivative; conversely, functions such as $\mid x \mid$ have weak derivatives despite lacking classical differentiability everywhere. Weak derivatives are linear, satisfy Leibniz rules when multiplying by smooth functions, but the classical chain rule can fail when composing with non-smooth maps.

> **Weak Derivatives of $\mid x \mid$**

> Compute weak derivatives of $u(x)=\mid x \mid$.
...
> Is $u(x)=\mid x-\tfrac{1}{2} \mid$ in $H^{1}(0,1)$?
...
> Let $f(t)=t^{2}$ and $u(x)=\mid x \mid$. Compute $(f\circ u)'$ weakly.

> Direct computation yields

> $$
> (f\circ u)' = -4\mid x \mid + 2\delta(x),
> $$

> whereas the classical chain rule would predict $2\mid x \mid \operatorname{sgn}(x)$. The discrepancy arises from the singularity at $x=0$.
...
For integer $k\ge 0$, the **Sobolev space** $H^{k}(\Omega)=W^{k,2}(\Omega)$ consists of functions $u\in L^{2}(\Omega)$ whose weak derivatives $D^{\alpha}u$ belong to $L^{2}(\Omega)$ for all $\mid \alpha \mid \le k$. The inner product

$$
\langle u,v\rangle_{H^{k}}=\sum_{\mid \alpha \mid \le k}\int_{\Omega} D^{\alpha}u(x)\,\overline{D^{\alpha}v(x)}\,dx
$$
...
> Let $u(x)=\mid x-\tfrac{1}{2} \mid$ and $\rho_{\epsilon}$ be a mollifier. Then $u_{\epsilon}=u*\rho_{\epsilon}\in C^{\infty}$ and $\|u-u_{\epsilon}\|_{H^{1}}\to 0$, illustrating density of smooth functions.
...
> $$
> \mid u(x) \mid =\left\mid \int_{0}^{x}u'(t)\,dt\right\mid \le \sqrt{x}\|u'\|_{L^{2}}\le \|u'\|_{L^{2}}.
> $$
...
> Let $u_{\epsilon}(x)=\mid \log x \mid^{1/4}\chi_{(0,\epsilon)}(x)$. Then $\|u_{\epsilon}\|_{H^{1}}\to C<\infty$ as $\epsilon\to 0$, but $\|u_{\epsilon}\|_{L^{4}}\to\infty$. Hence no embedding $H^{1}\hookrightarrow L^{p}$ exists at the critical exponent.
...
> $$
> \|u\|_{L^{2}(0,1)}^{2}\le \int_{0}^{1}\int_{0}^{1}\mid u'(t) \mid^{2}\,dt\,dx = \|u'\|_{L^{2}}^{2}.
> $$
...
> $$
> \gamma u(0)=\lim_{\epsilon\to 0^+}\frac{1}{2\epsilon}\int_{-\epsilon}^{\epsilon}\sqrt{\mid x \mid}\,dx=0,
> $$
...
> $$
> \iint_{(0,1)^{2}}\frac{\mid g(t)-g(s) \mid^{2}}{\mid t-s \mid^{2}}\,ds\,dt=\infty.
> $$
...
For $u(x) = x^2 \mid x \mid$, we have $u(x) = x^2 \cdot x = x^3$ for $x \ge 0$ and $u(x) = x^2 \cdot (-x) = -x^3$ for $x < 0$, so $u(x) = x^2 \mid x \mid = \operatorname{sgn}(x) x^3$.

For $x \neq 0$, the classical derivative is $u'(x) = 3x^2 \operatorname{sgn}(x) = 3x \mid x \mid$. Testing against $\phi \in C_c^{\infty}(\mathbb{R})$:

$$
\langle u', \phi \rangle = -\langle u, \phi' \rangle = -\int_{-\infty}^{\infty} x^2 \mid x \mid \phi'(x) \, dx.
$$

Integrating by parts on $(-\infty, 0)$ and $(0, \infty)$ separately:

$$
= -\int_{-\infty}^{0} -x^3 \phi'(x) \, dx - \int_{0}^{\infty} x^3 \phi'(x) \, dx = \int_{-\infty}^{0} 3x^2 \phi(x) \, dx - \int_{0}^{\infty} 3x^2 \phi(x) \, dx = \int_{-\infty}^{\infty} 3x \mid x \mid \phi(x) \, dx.
$$

So $u'(x) = 3x \mid x \mid$ weakly. For the second derivative, $u''(x) = 6 \mid x \mid$ classically for $x \neq 0$. Testing:

$$
\langle u'', \phi \rangle = -\langle u', \phi' \rangle = -\int_{-\infty}^{\infty} 3x \mid x \mid \phi'(x) \, dx.
$$

Integrating by parts gives $u''(x) = 6 \mid x \mid$ weakly.
...
For $u(x) = \mid x \mid^{-\alpha} \chi(\mid x \mid)$ with $\chi$ a smooth cutoff function supported in the unit ball, we compute norms using spherical coordinates.

For $\mid x \mid^{-\alpha}$ to be in $L^2$ near the origin, we need $\int_0^1 r^{n-1} r^{-2\alpha} \, dr < \infty$, which requires $\alpha < n/2$.

For $\mid \nabla u \mid = \alpha \mid x \mid^{-\alpha-1}$ to be in $L^2$, we need $\int_0^1 r^{n-1} r^{-2(\alpha+1)} \, dr < \infty$, which requires $\alpha < (n-2)/2$.
...
However, to show $u \in H^1$ even at the borderline, we need a more refined construction. Take $u(x) = \mid x \mid^{-(n-2)/2} (-\log \mid x \mid)^{-1/2} \chi(\mid x \mid)$ for $\mid x \mid < 1/2$, and $u(x) = 0$ otherwise.

Then $\mid \nabla u \mid^2 \sim \mid x \mid^{-n} (-\log \mid x \mid)^{-1}$ near the origin, and
...
But $\mid u \mid^p \sim \mid x \mid^{-n} (-\log \mid x \mid)^{-n/(n-2)}$, and
...
*(Hint: Use polar coordinates and Fourier series on the circle. For a function $g(\theta)$ on $\partial \Omega$, define its extension by $u(r,\theta) = \sum_{n \in \mathbb{Z}} c_n r^{\mid n \mid} e^{in\theta}$ where $c_n$ are the Fourier coefficients of $g$.)*
...
$$
u(r,\theta) = \sum_{n \in \mathbb{Z}} c_n r^{\mid n \mid} e^{in\theta}.
$$

This function is harmonic in $\Omega$ (since each term $r^{\mid n \mid} e^{in\theta}$ is harmonic) and satisfies $u(1,\theta) = g(\theta)$.

To show $u \in H^1(\Omega)$, compute:

$$
\|u\|_{H^1(\Omega)}^2 = \int_0^1 \int_0^{2\pi} \left( \mid u \mid^2 + \mid \partial_r u \mid^2 + \frac{1}{r^2} \mid \partial_\theta u \mid^2 \right) r \, dr \, d\theta.
$$

Using the orthogonality of $e^{in\theta}$:

$$
\|u\|_{H^1(\Omega)}^2 = 2\pi \sum_{n \in \mathbb{Z}} \mid c_n \mid^2 \int_0^1 \left( r^{2\mid n \mid + 1} + \mid n \mid^2 r^{2\mid n \mid - 1} + n^2 r^{2\mid n \mid - 1} \right) dr.
$$

For $n = 0$: $\int_0^1 r \, dr = 1/2$.

For $n \neq 0$: $\int_0^1 r^{2\mid n \mid + 1} \, dr = 1/(2\mid n \mid + 2)$, and $\int_0^1 \mid n \mid^2 r^{2\mid n \mid - 1} \, dr = \mid n \mid / 2$.

So:

$$
\|u\|_{H^1(\Omega)}^2 = 2\pi \left( \mid c_0 \mid^2 \cdot \frac{1}{2} + \sum_{n \neq 0} \mid c_n \mid^2 \left( \frac{1}{2\mid n \mid + 2} + \mid n \mid \right) \right) \le C \sum_{n \in \mathbb{Z}} (1 + \mid n \mid) \mid c_n \mid^2.
$$

The trace norm is:

$$
\|g\|_{H^{1/2}(\partial \Omega)}^2 = \sum_{n \in \mathbb{Z}} (1 + \mid n \mid) \mid c_n \mid^2,
$$

so $\|u\|_{H^1(\Omega)} \le C \|g\|_{H^{1/2}(\partial \Omega)}$, showing the extension operator is bounded.

Surjectivity follows because every $g \in H^{1/2}(\partial \Omega)$ has a Fourier expansion with coefficients satisfying $\sum (1 + \mid n \mid) \mid c_n \mid^2 < \infty$, and the extension $u$ defined above belongs to $H^1(\Omega)$ with $\gamma u = g$.
...
More precisely, if $u \in \bigcap_{k=0}^{\infty} H^k(\Omega)$, then for any multi-index $\alpha$, we have $D^{\alpha} u \in H^k$ for all $k$. Taking $k > n/2 + \mid \alpha \mid$, the Sobolev embedding gives $D^{\alpha} u \in C^0$, so $u \in C^{\infty}$.
