...
$$
p_{N,K}(\phi)=\max_{\vert \alpha \vert \le N}\sup_{x\in K} \vert D^{\alpha}\phi(x) \vert,
$$
...
$$
\vert \langle T,\phi\rangle \vert \le C \sum_{\vert \alpha \vert \le N}\sup_{x\in K}\vert D^{\alpha}\phi(x) \vert.
$$

The smallest such $N$ is the **order** of $T$. The Dirac mass $\delta_{a}$ defined by $\langle \delta_{a},\phi\rangle=\phi(a)$ satisfies $\vert \phi(a) \vert \le \sup \vert \phi \vert$, so $\delta_{a}$ has order $0$.

**Dirac Delta and Its Derivatives**

Show $\delta_{a}$ has order $0$ and compute its derivatives.

For $\phi\in\mathcal{D}$, $\vert \langle \delta_{a},\phi\rangle \vert =\vert \phi(a) \vert \le \sup\vert \phi \vert =p_{0}(\phi)$, so order $0$. Differentiating,
...
Show the Fourier series of $f(x)=\vert x \vert$ on $(-\pi,\pi)$ converges to $f$ in $\mathcal{D}'$.

Partial sums satisfy $\langle S_{N},\phi\rangle=\int_{-\pi}^{\pi}S_{N}(x)\phi(x)\,dx\to \int_{-\pi}^{\pi}\vert x \vert \phi(x)\,dx$ as $N\to\infty$, because $\phi$ decays near the endpoints. Thus $S_{N}\to f$ distributionally, even though Gibbs oscillations persist pointwise.
...
$$
\langle \partial^{\alpha} T,\phi\rangle := (-1)^{\vert \alpha \vert}\langle T,\partial^{\alpha}\phi\rangle.
$$
...
We have $\vert \langle \delta^{(n)},\phi\rangle \vert \le p_{n}(\phi)$. To show order $\ge n$, choose $\phi_{\epsilon}(x)=\epsilon^{n+1}\psi(x/\epsilon)$ with $\psi^{(n)}(0)=1$. Then $\vert \langle \delta^{(n)},\phi_{\epsilon}\rangle \vert =\epsilon$, while $p_{k}(\phi_{\epsilon})=O(\epsilon^{n+1-k})$. For $k<n$, $\vert \langle \delta^{(n)},\phi_{\epsilon}\rangle \vert /p_{k}(\phi_{\epsilon})\to \infty$, so no lower order bound exists.
...
Verify $-\Delta \Phi=\delta$ with $\Phi(x)=-\frac{1}{2\pi}\log\vert x \vert$.

For $\phi\in\mathcal{D}(\mathbb{R}^{2})$,

$$
\langle -\Delta \Phi,\phi\rangle=\langle \Phi,-\Delta \phi\rangle
 = -\frac{1}{2\pi}\int_{\mathbb{R}^{2}}\log\vert x \vert \Delta\phi(x)\,dx.
$$
...
Show $\vert x \vert^{\lambda-n}$ is homogeneous of degree $\lambda$.

For $t>0$,

$$
\langle \vert x \vert^{\lambda-n},\phi(x/t)\rangle
 = t^{\lambda}\langle \vert x \vert^{\lambda-n},\phi\rangle
$$

after the change $y=x/t$. Special cases include $\delta$ (degree $-n$) and $\vert x \vert^{2-n}$ (Green's function in $\mathbb{R}^{n}$, degree $2-n$).
...
The distribution $E(x,t)=H(t-\vert x \vert)$ satisfies $\partial_{tt}E-\partial_{xx}E=\delta(x)\delta(t)$. Verification follows by testing against $\phi$ and integrating by parts; $E$ describes the light cone in $1+1$ dimensions.

The wave equation fundamental solution illustrates how distributions handle propagation along characteristics: the support of $E$ is exactly the light cone $\vert x \vert \le t$, reflecting the finite speed of propagation. The discontinuity at $\vert x \vert =t$ is handled distributionally: the test function smooths the jump, making the calculation rigorous. This example connects distribution theory to the geometric structure of hyperbolic equations, showing how singular supports encode physical causality.
...
Consider the function $f(x) = \vert x \vert^{1/2}$ on $\mathbb{R}$. Show that $f$ defines a regular distribution, compute its first distributional derivative, and determine whether this derivative is itself a regular distribution. If not, characterize its singular structure.
...
Since $\vert x \vert^{1/2}$ is locally integrable, it defines a regular distribution $T_f$ via $\langle T_f, \phi \rangle = \int_{-\infty}^{\infty} \vert x \vert^{1/2} \phi(x) \, dx$.

For the derivative, we have

$$
\langle T_f', \phi \rangle = -\langle T_f, \phi' \rangle = -\int_{-\infty}^{\infty} \vert x \vert^{1/2} \phi'(x) \, dx.
$$
...
The derivative is $T_{f'}$ where $f'(x) = \frac{1}{2}\operatorname{sgn}(x)\vert x \vert^{-1/2}$ for $x \neq 0$. However, $\vert x \vert^{-1/2}$ is not locally integrable near $0$ (the integral $\int_0^{\epsilon} x^{-1/2} \, dx$ diverges), so $f'$ does not define a regular distribution.

The distributional derivative must be interpreted as a principal value or as a limit of regularizations. The singular structure at $x=0$ reflects the fact that $\vert x \vert^{1/2}$ has infinite slope at the origin: the classical derivative does not exist, and the distributional derivative reveals this singularity through non-integrability.
...
For $n \ge 3$, the fundamental solution of $-\Delta$ in $\mathbb{R}^n$ is $\Phi(x) = C_n \vert x \vert^{2-n}$ where $C_n$ is a dimensional constant. Verify that $-\Delta \Phi = \delta$ in $\mathcal{D}'(\mathbb{R}^n)$ and compute $C_n$ explicitly. Then show that the case $n=2$ (logarithmic fundamental solution) can be recovered as a limiting case.
...
For $n \ge 3$, we have $\Phi(x) = C_n \vert x \vert^{2-n}$. Computing the Laplacian away from the origin:

$$
\Delta \Phi(x) = C_n \Delta(\vert x \vert^{2-n}) = C_n (2-n) \nabla \cdot (x \vert x \vert^{-n}) = C_n (2-n)[n \vert x \vert^{-n} - n x \cdot x \vert x \vert^{-n-2}] = 0
$$

for $x \neq 0$, since $x \cdot x = \vert x \vert^2$.

To verify $-\Delta \Phi = \delta$, test against $\phi \in \mathcal{D}(\mathbb{R}^n)$:

$$
\langle -\Delta \Phi, \phi \rangle = \langle \Phi, -\Delta \phi \rangle = -C_n \int_{\mathbb{R}^n} \vert x \vert^{2-n} \Delta \phi(x) \, dx.
$$

Using integration by parts and the divergence theorem:

$$
-C_n \int_{\mathbb{R}^n} \vert x \vert^{2-n} \Delta \phi(x) \, dx = -C_n \lim_{\epsilon \to 0} \int_{\vert x \vert \ge \epsilon} \vert x \vert^{2-n} \Delta \phi(x) \, dx.
$$

Applying Green's identity:

$$
= -C_n \lim_{\epsilon \to 0} \left[ \int_{\vert x \vert \ge \epsilon} \Delta(\vert x \vert^{2-n}) \phi(x) \, dx + \oint_{\vert x \vert = \epsilon} \left( \vert x \vert^{2-n} \frac{\partial \phi}{\partial n} - \phi \frac{\partial}{\partial n}(\vert x \vert^{2-n}) \right) \, dS \right].
$$

Since $\Delta(\vert x \vert^{2-n}) = 0$ for $x \neq 0$, the volume integral vanishes. On the sphere $\vert x \vert = \epsilon$, we have $\frac{\partial}{\partial n}(\vert x \vert^{2-n}) = (2-n)\epsilon^{1-n}$. The boundary integral becomes:

$$
-C_n \lim_{\epsilon \to 0} \oint_{\vert x \vert = \epsilon} \left[ \vert x \vert^{2-n} \frac{\partial \phi}{\partial n} - \phi (2-n)\epsilon^{1-n} \right] \, dS.
$$

As $\epsilon \to 0$, the first term vanishes (since $\vert x \vert^{2-n} = \epsilon^{2-n}$ and the surface area is $O(\epsilon^{n-1})$), and the second term gives:

$$
C_n (2-n) \lim_{\epsilon \to 0} \epsilon^{1-n} \oint_{\vert x \vert = \epsilon} \phi(x) \, dS = C_n (2-n) \lim_{\epsilon \to 0} \epsilon^{1-n} \cdot \text{Area}(S^{n-1}) \cdot \epsilon^{n-1} \cdot \phi(0) = C_n (2-n) \text{Area}(S^{n-1}) \phi(0).
$$
...
For the $n=2$ case, we cannot use $\vert x \vert^{2-n}$ directly since it becomes $\vert x \vert^0 = 1$ (a constant). However, taking the limit $n \to 2$ in the expression for $C_n \vert x \vert^{2-n}$ and using the asymptotic expansion of the Gamma function, we recover the logarithmic fundamental solution $\Phi(x) = -\frac{1}{2\pi} \log \vert x \vert$.
