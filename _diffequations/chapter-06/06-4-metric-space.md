...
> **Tangent Space Identification on $S^2$**

> To explicitly identify the tangent space $T_g \text{Met}(M)$ for $M = S^2$ with the standard round metric, we begin with the round metric $g = d\theta^2 + \sin^2\theta \, d\phi^2$ on $S^2$ with coordinates $(\theta, \phi)$.

> The tangent space $T_g \text{Met}(S^2) \cong \Gamma(S^2 T^*S^2)$ consists of all symmetric $(0,2)$-tensor fields on $S^2$. A general element $h \in T_g \text{Met}(S^2)$ can be written as $h = h_{\theta\theta} d\theta^2 + 2h_{\theta\phi} d\theta d\phi + h_{\phi\phi} d\phi^2$ where $h_{\theta\theta}$, $h_{\theta\phi}$, and $h_{\phi\phi}$ are smooth functions on $S^2$.

> Consider the variation $h = \epsilon \sin^2\theta \, d\phi^2$ for small $\epsilon$. This represents a metric $g_\epsilon = d\theta^2 + (1+\epsilon)\sin^2\theta \, d\phi^2$, which is an ellipsoidal deformation of the sphere. This variation changes the geometry (not just coordinates) since it alters the curvature, demonstrating how the tangent space encodes genuine geometric deformations.

> **$L^2$ Metric Computation on Torus**

> To compute the $L^2$ inner product of two metric variations on the torus $T^2$, we begin with the flat torus metric $g = dx^2 + dy^2$ on $T^2 = S^1 \times S^1$ with coordinates $(x,y) \in [0,2\pi)^2$.

> Consider two variations $h = \cos x \, dx^2$ and $k = \sin y \, dy^2$. The $L^2$ inner product is $\langle h, k \rangle_{L^2(g)} = \int_{T^2} \text{tr}_g(h \otimes k) \, d\text{vol}_g$. Computing the trace gives $\text{tr}_g(h \otimes k) = g^{xx} g^{xx} h_{xx} k_{xx} + g^{yy} g^{yy} h_{yy} k_{yy} = \cos x \cdot 0 + 0 \cdot \sin y = 0$. Since $h$ has only $xx$ component and $k$ has only $yy$ component, they are orthogonal. With the volume form $d\text{vol}_g = dx \wedge dy$, we find $\langle h, k \rangle_{L^2(g)} = \int_0^{2\pi} \int_0^{2\pi} 0 \, dx \, dy = 0$. Variations in orthogonal directions are $L^2$-orthogonal, demonstrating how the $L^2$ metric encodes the geometric structure of the space of metrics.

> **Berger-Ebin Decomposition**

> To decompose a metric variation $h$ on $\mathbb{R}^2$ into transverse and longitudinal parts, we begin with the flat metric $g = dx^2 + dy^2$ on $\mathbb{R}^2$ and the given variation $h = x \, dx^2 + y \, dy^2$.

> The longitudinal part is generated by a vector field $X = X^x \partial_x + X^y \partial_y$ such that $\mathcal{L}_X g = \nabla_i X_j + \nabla_j X_i$. In flat coordinates, $(\mathcal{L}_X g)_{ij} = \partial_i X_j + \partial_j X_i$, giving $(\mathcal{L}_X g)_{xx} = 2\partial_x X^x$, $(\mathcal{L}_X g)_{yy} = 2\partial_y X^y$, and $(\mathcal{L}_X g)_{xy} = \partial_x X^y + \partial_y X^x$.

> We want $h = h^{\perp} + \mathcal{L}_X g$ with $\text{div}_g h^{\perp} = 0$. The divergence condition $(\text{div}_g h^{\perp})_j = -g^{ik} \partial_i h^{\perp}_{kj} = 0$ gives $-\partial_x h^{\perp}_{xx} - \partial_y h^{\perp}_{yx} = 0$ for $j=x$ and $-\partial_x h^{\perp}_{xy} - \partial_y h^{\perp}_{yy} = 0$ for $j=y$. A naive choice $X^x = \frac{x^2}{4}$, $X^y = \frac{y^2}{4}$ gives $\mathcal{L}_X g = \frac{x}{2} dx^2 + \frac{y}{2} dy^2$ and $h^{\perp} = \frac{x}{2} dx^2 + \frac{y}{2} dy^2$, but verification shows $\text{div}_g h^{\perp} = -1 \neq 0$. The correct decomposition requires solving a Poisson equation for $X$, demonstrating how the Berger-Ebin decomposition systematically separates geometric from coordinate changes.

> **Diffeomorphism Action via Pullback**

> To show that the diffeomorphism $\varphi(x,y) = (2x, 2y)$ on $\mathbb{R}^2$ acts on metrics via pullback, we begin with the original metric $g = dx^2 + dy^2$. The pullback is $\varphi^* g = d(2x)^2 + d(2y)^2 = 4dx^2 + 4dy^2 = 4g$.

> The pullback metric is a uniform scaling. The geometry (flatness) is preserved, but distances are scaled by a factor of 2. All metrics of the form $\lambda g$ for $\lambda > 0$ lie in the same orbit under the scaling subgroup of $\text{Diff}(\mathbb{R}^2)$. The moduli space identifies all conformally equivalent flat metrics, demonstrating how diffeomorphisms act on the space of metrics while preserving geometric structure.

> **Einstein-Hilbert Functional First Variation**

> To compute the first variation of $\mathcal{E}(g) = \int_M R_g \, d\text{vol}_g$ for a 2D surface, we note that for a 2D metric, $R = 2K$ where $K$ is the Gaussian curvature.

> The first variation of the scalar curvature is $\delta R = -\Delta_g \text{tr}_g h + \text{div}_g(\text{div}_g h) - \langle \text{Ric}, h \rangle$, and the volume form variation is $\delta(d\text{vol}_g) = \frac{1}{2} \text{tr}_g h \, d\text{vol}_g$. Combining these gives $\delta \mathcal{E}(h) = \int_M \left[ \delta R \, d\text{vol}_g + R \, \delta(d\text{vol}_g) \right] = \int_M \left[ (-\Delta_g \text{tr}_g h + \text{div}_g(\text{div}_g h) - \langle \text{Ric}, h \rangle) + \frac{R}{2} \text{tr}_g h \right] d\text{vol}_g$.

> Integrating by parts, the divergence terms integrate to zero (assuming compact $M$ or appropriate boundary conditions), leaving $\delta \mathcal{E}(h) = \int_M \left\langle \frac{R}{2} g - \text{Ric}, h \right\rangle d\text{vol}_g$. In 2D, $\text{Ric} = \frac{R}{2} g$, so the Einstein tensor vanishes. This reflects the fact that all 2D metrics are locally conformally flat, demonstrating how the dimension affects the structure of geometric functionals.

> **Ebin Slice for Flat Torus**

> To construct an explicit slice for the flat torus metric, we begin with $M = T^2$ and $g = dx^2 + dy^2$ with $(x,y) \in [0,2\pi)^2$. The isometry group is $\text{Iso}(g) = T^2 \rtimes D_4$ (translations and discrete rotations/reflections).

> The slice $\mathcal{S}_g$ consists of metrics $\tilde{g}$ such that $\text{div}_g (\tilde{g} - g) = 0$ (divergence-free condition) and $\tilde{g}$ is close to $g$ in $H^k$ topology. A metric in the slice can be written as $\tilde{g} = g + h^{\perp}$ where $h^{\perp}$ satisfies $\text{div}_g h^{\perp} = 0$. Near $g$, the slice provides a local coordinate chart for the moduli space $\mathcal{M}$, parameterizing geometries (not just metrics) up to diffeomorphism, demonstrating how the Ebin slice theorem resolves the gauge redundancy in the space of metrics.

> **Moduli Space Singularity for Round Sphere**

> To analyze the singularity in $\mathcal{M}$ corresponding to the round sphere metric, we begin with $M = S^2$ and $g = d\theta^2 + \sin^2\theta \, d\phi^2$. The isometry group is $\text{Iso}(g) = SO(3)$ (3-dimensional rotation group).

> The orbit $\mathcal{O}_g$ has codimension 3 in $\text{Met}(S^2)$ due to the 3-dimensional isometry group. The moduli space $\mathcal{M}$ has a singularity at $[g]$ because the action of $\text{Diff}(S^2)$ is not free at $g$, and the slice theorem gives $\text{Diff}(S^2) \times_{SO(3)} \mathcal{S}_g$. The moduli space is stratified by the dimension of the isometry group. Metrics with larger isometry groups lie in lower-dimensional strata, demonstrating how symmetries create singularities in the moduli space.

> **Ricci Flow as Gradient Flow**

> To show that Ricci flow is (up to diffeomorphism) the gradient flow of Perelman's $\mathcal{F}$-functional, we begin with Perelman's $\mathcal{F}$-functional $\mathcal{F}(g,f) = \int_M (R + \mid \nabla f \mid^2) e^{-f} \, d\text{vol}_g$ where $f$ is a function on $M$.

> The $L^2$ gradient of $\mathcal{F}$ with respect to $g$ (fixing $f$) is $\nabla_{L^2} \mathcal{F} = \text{Ric} + \nabla^2 f$. The normalized Ricci flow is $\frac{\partial g}{\partial t} = -2(\text{Ric} - \lambda g)$ where $\lambda$ is chosen to preserve volume. Under the diffeomorphism gauge, the Ricci flow can be interpreted as gradient descent on the moduli space $\mathcal{M}$ with respect to Perelman's entropy functionals. This interpretation explains why Ricci flow tends to evolve metrics toward constant curvature (critical points of the functional), connecting the geometric evolution to variational principles.
