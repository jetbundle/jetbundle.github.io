...
$$
\mid \mid u(\cdot, t) - v(\cdot, t) \mid \mid_{L^1} \leq \mid \mid u_0 - v_0 \mid \mid_{L^1}
$$
...
> Consider $u(x,t)$ and $v(x,t)$ with initial data $u_0$ and $v_0$. The $L^1$ distance at time $t$ is $\int \mid u(x,t) - v(x,t) \mid dx$.

> For smooth solutions, we can differentiate: $\frac{d}{dt} \int \mid u - v \mid dx = \int \text{sgn}(u-v) (\partial_t u - \partial_t v) dx = -\int \text{sgn}(u-v) (\partial_x F(u) - \partial_x F(v)) dx$.
