...
This linear hyperbolic equation governs the evolution of the amplitude $a(x)$ along the flow determined by $\nabla S$. As discussed in Evans (2010), this equation implies the conservation of energy flux along the rays. Specifically, the quantity $a^2 \vert \nabla S \vert$ is conserved along the characteristics of the Eikonal equation.

### Hamilton-Jacobi Theory

To solve the Eikonal equation $\vert \nabla S \vert^2 = n^2(x)$, we employ the method of characteristics, framing the problem within Hamilton-Jacobi mechanics. We define the Hamiltonian $H(x,p)$ on the cotangent bundle $T^*\Omega$ (phase space):

$$
H(x,p) = \frac{1}{2} \left( \vert p \vert^2 - n^2(x) \right)
$$
...
$$
\lim_{x \to \text{caustic}} \vert u(x) \vert = \infty
$$
...
$$
-\epsilon \Delta u^\epsilon + \vert \nabla u^\epsilon \vert = n(x)
$$
...
We seek an asymptotic solution in the form:

$$
u(x,z) \sim a(x,z) e^{ik S(x,z)}
$$

The Eikonal equation gives $\vert \nabla S \vert^2 = n^2(z) = (1 + \frac{1}{2}z^2)^2$, so $S_z^2 + S_x^2 = 1 + z^2 + \frac{1}{4}z^4$. Assuming a plane wave in $x$: $S(x,z) = x + T(z)$, we get $T'(z)^2 = z^2 + \frac{1}{4}z^4$, so $T'(z) = z\sqrt{1 + \frac{1}{4}z^2}$.
...
For radio wave propagation in Earth's ionosphere with $n(z) = \sqrt{1 - \frac{z^2}{h^2}}$, the Eikonal equation is $\vert \nabla S \vert^2 = 1 - \frac{z^2}{h^2}$.
...
2. **Jacobi Field:** $J(t) = \frac{d}{ds}\gamma(t,su)\vert_{s=0}$
...
For $\vert \nabla u \vert = 1$ in the unit disk with $u=0$ on the boundary, the classical method of characteristics gives straight lines from the boundary that intersect at the center, making the solution multi-valued.

The viscosity solution is $u(x) = \min_{y\in\partial B} \vert x-y \vert = 1 - \vert x \vert$. This satisfies the viscosity conditions: for any test function $\phi$ touching from above, $\vert \nabla\phi(x_0) \vert \leq 1$, and for any $\psi$ touching from below, $\vert \nabla\psi(x_0) \vert \geq 1$.

The viscous approximation $-\epsilon\Delta u^\epsilon + \vert \nabla u^\epsilon \vert = 1$ converges uniformly to $u^\epsilon \to 1-\vert x \vert$ as $\epsilon \to 0$.
...
For $\vert \nabla u \vert = n(x)$ where $n(x,y) = \sqrt{1-x^2}$, the rays are cycloids $(p_0\sin t, p_y t)$.
...
Show that the Eikonal equation $\vert \nabla S \vert^2 = n^2(x)$ is equivalent to Fermat's principle of least time: light rays follow paths that minimize the optical path length $\int n(x) ds$.
...
Fermat's principle states that light rays follow paths $\gamma(t)$ that minimize the optical path length $L[\gamma] = \int_{\gamma} n(x) ds = \int_{t_0}^{t_1} n(\gamma(t)) \vert \dot{\gamma}(t) \vert dt$.

Using the calculus of variations, the Euler-Lagrange equations for this functional give:

$$\frac{d}{dt}\left(\frac{n(\gamma) \dot{\gamma}}{\vert \dot{\gamma} \vert}\right) = \nabla n(\gamma) \vert \dot{\gamma} \vert$$

For a path parameterized by arc length ($\vert \dot{\gamma} \vert = 1$), this becomes:

$$\frac{d}{ds}(n \dot{\gamma}) = \nabla n$$

which is exactly the ray equation derived from Hamilton's equations with the Eikonal equation.

The phase $S(x)$ is the optical path length from a source point, so $\nabla S$ points in the direction of the ray, and $\vert \nabla S \vert = n$ is the Eikonal equation.
...
Compute the amplitude divergence near a fold caustic and show that it scales as $a \sim \vert x - x_c \vert^{-1/6}$ where $x_c$ is the caustic point.
...
Near the caustic, we can expand $\gamma(t, \alpha) = \gamma_c + \frac{\partial \gamma}{\partial t}\vert_c (t - t_c) + \frac{1}{2}\frac{\partial^2 \gamma}{\partial \alpha^2}\vert_c (\alpha - \alpha_c)^2 + \cdots$ where the subscript $c$ denotes evaluation at the caustic.

The Jacobian near the caustic is $J \sim \vert \alpha - \alpha_c \vert$ (since the first-order term in $\alpha$ vanishes). The distance from the caustic is $\vert x - x_c \vert \sim (\alpha - \alpha_c)^2$, so $J \sim \vert x - x_c \vert^{1/2}$.

The amplitude satisfies $a^2 \vert \nabla S \vert J = \text{constant}$ (conservation of energy flux), so $a \sim J^{-1/2} \sim \vert x - x_c \vert^{-1/4}$.

However, this is the divergence in the geometric optics approximation. The actual field near a fold caustic is described by the Airy function, which gives $a \sim \vert x - x_c \vert^{-1/6}$ after proper uniform asymptotics, showing that the geometric optics prediction overestimates the divergence.

</details>

### Challenge 3: Viscosity Solutions and the Hopf-Lax Formula

For the Eikonal equation $\vert \nabla u \vert = n(x)$ with boundary condition $u = 0$ on $\partial \Omega$, the viscosity solution can be expressed using the Hopf-Lax formula:

$$u(x) = \inf_{y \in \partial \Omega} \left\{ \int_0^1 n(\gamma(s)) \vert \dot{\gamma}(s) \vert ds : \gamma(0) = y, \gamma(1) = x \right\}$$
...
Use this to solve $\vert \nabla u \vert = 1$ in the unit disk with $u = 0$ on the boundary, and show that $u(x) = 1 - \vert x \vert$.
...
For $\vert \nabla u \vert = 1$ in the unit disk with $u = 0$ on the boundary, in a uniform medium ($n = 1$), the minimal path is a straight line. The distance from a point $x$ to the boundary is $1 - \vert x \vert$ (the distance to the nearest boundary point), so $u(x) = 1 - \vert x \vert$.

To verify the viscosity conditions, consider a test function $\phi$ such that $u - \phi$ has a local maximum at $x_0$. Since $u(x) = 1 - \vert x \vert$, we have $\phi(x) \geq 1 - \vert x \vert$ near $x_0$ with equality at $x_0$.

At $x_0 \neq 0$, $u$ is smooth and $\vert \nabla u \vert = 1$, so if $\phi$ touches from above, we must have $\vert \nabla \phi(x_0) \vert \leq 1$ (otherwise $\phi$ would decrease faster than $u$, violating the maximum condition).

At $x_0 = 0$, $u$ has a corner. For any test function $\phi$ touching from above, the gradient condition $\vert \nabla \phi(0) \vert \leq 1$ must hold in all directions, which is satisfied since $u$ is the distance function.

For a test function $\psi$ touching from below, we need $\vert \nabla \psi(x_0) \vert \geq 1$, which is also satisfied. Therefore, $u(x) = 1 - \vert x \vert$ is the unique viscosity solution.
